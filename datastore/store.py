...
class Store:
    def __init__(self, db_path: str = "taw.db", artifacts_dir: str = "artifacts"):
    ...
    def ensure_artifacts_dir(self) -> str:
        Path(self.artifacts_dir).mkdir(parents=True, exist_ok=True)
    ...
    def log_run(
        ...
        cur = self.conn.execute(
            """INSERT INTO runs(task_id, step, tool, inputs_json, outputs_json, status)
               VALUES(?,?,?,?,?,?)""",
            (task_id, step, tool,
    ...
    # --- Optional helpers for quick checks (handy during the interview demo) ---

    def list_tasks(self) -> List[dict]:
        cur = self.conn.execute(
            "SELECT id, created_at, prompt, objective, params_json, status FROM tasks ORDER BY id DESC"
        )
        rows = cur.fetchall()
        return [
            {
    ...
    def get_task(self, task_id: int) -> Optional[dict]:
        cur = self.conn.execute(
            "SELECT id, created_at, prompt, objective, params_json, status FROM tasks WHERE id=?",
            (task_id,),
        )
        r = cur.fetchone()
        if not r:
    ...
    def list_runs(self, task_id: int) -> List[dict]:
        cur = self.conn.execute(
            """SELECT id, step, tool, inputs_json, outputs_json, started_at, ended_at, status
               FROM runs WHERE task_id=? ORDER BY id ASC""",
            (task_id,),
        )
        rows = cur.fetchall()
        return [
    ...
    def list_metrics(self, task_id: int) -> List[dict]:
        cur = self.conn.execute(
            "SELECT id, name, value FROM metrics WHERE task_id=? ORDER BY id ASC", (task_id,)
        )
        rows = cur.fetchall()
